/**
 * Gemini AI Service for BaZi Consultant
 * Uses Google Gemini models via Gemini API
 */
...
## CÂU HỎI CỦA NGƯỜI DÙNG
 ...
 3. CUỐI CÙNG LÀ PHẦN [FOLLOW_UP] VỚI 3-5 CÂU HỎI GỢI MỞ.
    ...
    async generateCompletion(prompt, personaId = 'huyen_co') {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                console.log(`[Gemini/Completion] Attempt ${attempt}/${this.maxRetries}...`);
                ...
                console.log(`[Gemini/Completion] Success on attempt ${attempt}`);
                ...
                lastError = error;
                console.error(`[Gemini/Completion] Attempt ${attempt} failed:`, error.message);

                if (attempt < this.maxRetries && this.isRetryableError(error)) {
                    const delay = Math.pow(2, attempt) * 1000;
                    console.log(`[Gemini/Completion] Retrying in ${delay}ms...`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
            }
        }

        console.error('[Gemini/Completion] All attempts failed, returning fallback');
    ...
    async generateMatchingAnswer(person1Ctx, person2Ctx, relationshipType = 'romance', personaId = 'huyen_co') {
        ...
        BẠN PHẢI TRẢ VỀ DUY NHẤT MỘT ĐỐI TƯỢNG JSON theo cấu trúc sau, không kèm bất kỳ văn bản nào khác:
        {
          ...
          "breakdown": {
            "element": { "score": number (max 30), "maxScore": 30, "description": "...", "quality": "..." },
            "ganzhi": { "score": number (max 25), "maxScore": 25, "details": [ { "type": "positive"|"negative", "text": "..." } ], "quality": "..." },
            "shishen": { "score": number (max 25), "maxScore": 25, "details": [ { "type": "positive"|"negative", "text": "..." } ], "quality": "..." },
            "star": { "score": number (max 20), "maxScore": 20, "details": [ { "type": "positive"|"negative", "text": "..." } ], "quality": "..." }
          },
          "aspects": [
            { "type": "romance", "icon": "💕", "title": "Tình Cảm", "score": number, "description": "..." },
            { "type": "communication", "icon": "💬", "title": "Giao Tiếp", "score": number, "description": "..." },
            { "type": "children", "icon": "👶", "title": "Con Cái", "score": number, "description": "..." },
            { "type": "finance", "icon": "💰", "title": "Tài Chính", "score": number, "description": "..." },
            { "type": "lifestyle", "icon": "🏠", "title": "Lối Sống", "score": number, "description": "..." }
          ],
          "advice": [ { "type": "positive"|"neutral"|"warning"|"tip", "text": "..." } ],
          "suggestedQuestions": [ "...", "...", "...", "...", "..." ]
        }`;
        ...
        try {
            ...
            try {
                ...
                console.log('[Gemini/Matching] Successfully parsed and validated JSON response');
                return parsedResult;
            } catch (jsonError) {
                console.error('[Gemini/Matching] JSON parsing failed:', jsonError.message);
                console.error('[Gemini/Matching] Returning fallback response');
            ...
            console.error('[Gemini/Matching] Fatal error:', error);
